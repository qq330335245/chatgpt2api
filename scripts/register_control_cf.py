...
OUT = Path(r'G:\project\chatgpt2api\data\register_control_cf_result.json')
...
for attempt, proxy in enumerate(proxies, 1):
    ...
    try:
        ...
        payload = {'ok': True, 'attempt': attempt, 'proxy': proxy, 'main_email': 'cloudflare_control', 'result': result, 'finished_at': datetime.now().isoformat()}
        ...
        registrar.close()
        time.sleep(2)
payload = {'ok': False, 'main_email': 'cloudflare_control', 'error': str(last_err), 'finished_at': datetime.now().isoformat()}
OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
